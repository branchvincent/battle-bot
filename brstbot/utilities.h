...
void run_command(String key, String value) {
  if (key.equals("speed") || key.equals("s")) {   // e.g., user entered  "speed:37"  in serial monitor
    ...
    BRSTbot::b.setSpeed(value.toInt());
    log("Changing speed to: ", value);

  } else if (key.equals("bias") || key.equals("b")) {

    BRSTbot::b.setMotorBias(value.toFloat());
    log("Setting bias to: ", value);

  } else if (key.equals("dir") || key.equals("d")) {

    if (value.equals("forward") || value.equals("f")) { BRSTbot::b.startMotors(); }
    else if (value.equals("back") || value.equals("r") || value.equals("b")) { BRSTbot::b.reverseMotors(); }
    else if (value.equals("stop") || value.equals("s")) { BRSTbot::b.stopMotors(); }
    log("Setting motor direction to: ", value);

  } else if (key.equals("rotate") || key.equals("r")) {

    if (value.equals("left") || value.equals("l")) { BRSTbot::b.rotateLeft(); }
    else if (value.equals("right") || value.equals("r")) { BRSTbot::b.rotateRight(); }
    else if (value.equals("straight") || value.equals("s")) { BRSTbot::b.rotateStraight(); }
